...
const defaultState2 = (): SnowflakeAppState => {
  return {
    name: 'Enter Name',
    title: 'Staff Engineer',
    milestoneByTrack: {
...
const stateToHashEvan = (state: SnowflakeAppState) => {
  if (!state || !state.milestoneByTrack) return null
...
class SnowflakeApp extends React.Component<Props, SnowflakeAppState> {
  ...
  componentDidUpdate() {
    ...
    console.log("EVAN componoent did update")
    console.log(hash)
    //if (hash) window.location.replace(`#${hash}`)
  }

  componentDidMount() {
    console.log("EVAN comp");
    console.log(this.props.posts);
    var state;
    if (this.props.posts != undefined) {
      // String of the values
      state = hashToState(this.props.posts.content);
    } else {
  ...
  handleTrackMilestoneChange(trackId: TrackId, milestone: Milestone) {
    ...
    console.log("EVAN handleTrackMilestoneChange")
    console.log({ milestoneByTrack, focusedTrackId: trackId, title })

    this.setState({ milestoneByTrack, focusedTrackId: trackId, title })
    this.props.callbackFromParent(stateToHashEvan(this.state))
